<section
    @class([
        'bottom-28 left-0 fixed z-40 flex flex-col items-start group/message',
        $class,
    ])
...
{{ $attributes }}>
    <div class="bg-info-400 h-14 px-5 flex items-center justify-between gap-x-3 message-warning cursor-pointer relative">
        ...
        <x-atoms.text class="text-light text-nowrap group-[.active]/message:hidden">Site fictício</x-atoms.text>

        <div class="w-6 h-6 flex border border-info-100 items-center justify-center bg-info-300 text-light rounded-full absolute -top-3 -right-3 group-[.active]/message:hidden">
            <x-icons.close />
        </div>
    </div>
    <div class="overflow-hidden w-full shadow-depth group-[.active]/message:-translate-x-full group-[.active]/message:opacity-0 transition-all duration-700">
